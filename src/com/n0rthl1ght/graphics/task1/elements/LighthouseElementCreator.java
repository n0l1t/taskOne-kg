package com.n0rthl1ght.graphics.task1.elements;

import java.awt.*;

public class LighthouseElementCreator {
    private final Color LHWhite = new Color(255,255,255);
    private final Color LHWRed = new Color(218,93,82);
    public static Lighthouse element1R(){
        int[] x = {560,565,645,650};
        int[] y = {400,380,380,400};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
    public static Lighthouse element1W(){
        int[] x = {570,575,635,640};
        int[] y = {380,350,350,380};
        return new Lighthouse(x,y,new Color(255,255,255));
    }
    public static Lighthouse element2R(){
        int[] x = {575,580,630,635};
        int[] y = {350,320,320,350};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
    public static Lighthouse element2W(){
        int[] x = {580,585,625,630};
        int[] y = {320,290,290,320};
        return new Lighthouse(x,y,new Color(255,255,255));
    }
    public static Lighthouse element3R(){
        int[] x = {585,590,620,625};
        int[] y = {290,260,260,290};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
    public static Lighthouse element3W(){
        int[] x = {590,595,615,620};
        int[] y = {260,230,230,260};
        return new Lighthouse(x,y,new Color(255,255,255));
    }
    public static Lighthouse element4R(){
        int[] x = {595,580,630,615};
        int[] y = {230,220,220,230};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
    public static Lighthouse element4W(){
        int[] x = {590,590,620,620};
        int[] y = {220,210,210,220};
        return new Lighthouse(x,y,new Color(255,255,255));
    }
    public static Lighthouse element4E(){
        int[] x = {590,590,620,620};
        int[] y = {210,180,180,210};
        return new Lighthouse(x,y,new Color(185,223,226));
    }
    public static Lighthouse element5R(){
        int[] x = {590,585,625,620};
        int[] y = {180,175,175,180};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
    public static Lighthouse element6R(){
        int[] x = {585,605,625};
        int[] y = {175,160,175};
        return new Lighthouse(x,y,new Color(218,93,82));
    }
}
